...
var strTitle = "大图切割机";//localize("$$$/JavaScripts/ExportLayersToFiles/Title=切割");
var strButtonRun = "运行";//localize("$$$/JavaScripts/ExportLayersToFiles/Run=運行");
var strButtonCancel = "取消";//localize("$$$/JavaScripts/ExportLayersToFiles/Cancel=取消");
var strHelpText = "请设定图片的保存格式和路径.";//localize("$$$/JavaScripts/ExportLayersToFiles/Help=Please specify the format and location for saving each layer as a file.");
var strLabelDestination = "图片保存路径";//localize("$$$/JavaScripts/ExportLayersToFiles/Destination=Destination:");
var strButtonBrowse = "选择";//localize("$$$/JavaScripts/ExportLayersToFiles/Browse=&Browse...");
var strLabelFileNamePrefix = "图片名前缀";//localize("$$$/JavaScripts/ExportLayersToFiles/FileNamePrefix=File Name Prefix:");
var strCheckboxVisibleOnly = "只导出可见层";//localize("$$$/JavaScripts/ExportLayersToFiles/VisibleOnly=&Visible Layers Only");
var strCheckboxFastAndSimple = "不删除隐藏层";//localize("$$$/JavaScripts/ExportLayersToFiles/FastAndSimple=&Fast Mode (Don't remove hidden layers)");
var strLabelFileType = "图片格式";//localize("$$$/JavaScripts/ExportLayersToFiles/FileType=File Type:");
var strCheckboxIncludeICCProfile = "包含ICC Profile";//localize("$$$/JavaScripts/ExportLayersToFiles/IncludeICC=&Include ICC Profile");
...
var strAlertSpecifyDestination = "请指定存放路径.";//localize("$$$/JavaScripts/ExportLayersToFiles/SpecifyDestination=Please specify destination.");
var strAlertDestinationNotExist = "存放路径不存在";//localize("$$$/JavaScripts/ExportLayersToFiles/DestionationDoesNotExist=Destination does not exist.");
var strTitleSelectDestination = "选择存放路径";//localize("$$$/JavaScripts/ExportLayersToFiles/SelectDestination=Select Destination");
var strAlertDocumentMustBeOpened = "请打开一个文档";//localize("$$$/JavaScripts/ExportLayersToFiles/OneDocument=You must have a document open to export!");
...
var strAlertWasSuccessful = "成功";//localize("$$$/JavaScripts/ExportLayersToFiles/Success= was successful.");
...
//            setInvisibleAllArtLayers(duppedDocument);
            sliceLayerAndExport(duppedDocument, exportInfo);
...
function settingDialog(exportInfo) {
	...
	dlgMain.grpTileLine = dlgMain.grpTopLeft.add("group");
	dlgMain.grpTileLine.orientation = 'row';
	dlgMain.grpTileLine.alignChildren = 'center';
	dlgMain.grpTileLine.add("statictext", undefined, "tileW");
	dlgMain.tileWidthText = dlgMain.grpTileLine.add("edittext", undefined, exportInfo.tileWidth.toString());
	dlgMain.grpTileLine.add("statictext", undefined, "tileH");
	dlgMain.tileHeightText = dlgMain.grpTileLine.add("edittext", undefined, exportInfo.tileHeight.toString());
...
function initExportInfo(exportInfo) {
    ...
    exportInfo.tileWidth = 300;
    exportInfo.tileHeight = 300;
...
///////////////////////////////////////////////////////////////////////////////
function sliceLayerAndExport(doc, exportInfo){
	//圖片尺寸(像素)
	var picW = doc.width;
	var picH = doc.height;
	var tileW = exportInfo.tileWidth;
	var tileH = exportInfo.tileHeight;
	
	var r = Math.ceil(picH/tileH);
	var c = Math.ceil(picW/tileW);
	
    var l0 = doc.activeLayer;
    //
    var tmpDoc = app.documents.add(tileW, tileH);
    //
    for(var j = 0 ; j < r ; j++){
        for(var i = 0 ; i < c; i++){
            var x = i * tileW; //edited
            var y = j * tileH; //here
            app.activeDocument = doc;
            //select
            doc.selection.select([
                                  [Math.min(x, doc.width)		,Math.min(y, doc.height)],
                                  [Math.min(x+tileW, doc.width)	,Math.min(y, doc.height)],
                                  [Math.min(x+tileW, doc.width)	,Math.min(y+tileH, doc.height)],
                                  [Math.min(x, doc.width)		,Math.min(y+tileH, doc.height)]
                                ]);
            //new layer via cut
            doc.selection.copy();
            ...
            tmpDoc.paste();
            tmpDoc.activeLayer.translate(-(tmpDoc.width - (bounds[2] - bounds[0]))/2, -(tmpDoc.height - (bounds[3] - bounds[1]))/2);
            
            saveFile(tmpDoc, exportInfo.fileNamePrefix+ "_" + i+"_"+j, exportInfo);
